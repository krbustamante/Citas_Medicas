...
class _MyAppState extends State<lostpass> {
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: Center(
            ...
            child: ListView(
          //Creamos un contenedor que va poder hacer scroll
          children: <Widget>[
            //creamos una lista que pondra mas widgets uno tras otro
            head(),
            correo(),
            mensaje(),
  ...
  Future<List> _verifyEmail() async {
    ...
    final response = await http.post(
        Uri.parse('http://krbustamante.byethost7.com/php/getemail.php'),
        body: {
  ...
  Future<void> _sendemail() async {
    final response = await http.post(
        Uri.parse('http://citasmedicas.atwebpages.com/sendemail.php'),
        body: {
          "email": email_txt.text,
        });
    var datauser = json.decode(response.body);
  ...
  Widget head() {
    return Container(
      padding: EdgeInsets.fromLTRB(
          0, 20.0, 0, 0), //Padding con los valores left, top, rigt, button
  ...
  Widget btnsendcode() {
    return Container(
      ...
      child: ElevatedButton(
        ...
        child: Text('Verificar'), //Texto del boton
        style: ElevatedButton.styleFrom(
          //Definimos estilos
          backgroundColor: Color.fromARGB(255, 0, 164, 65), //Color del boton
        ),
